...
class SQLAgentGraphBuilder(TSModel):
    ...
    def should_enrich(self, state: SQLAssistantState):
        logging.info("🔍 [GRAPH_BUILDER] Evaluating should_enrich routing decision")
        logging.info(f"🔍 [GRAPH_BUILDER] Question: {state.input}")
        
        # CRITICAL: Check for irrelevant questions first (circuit breaker condition)
        if hasattr(state, 'grade') and state.grade:
            logging.info("🔍 [GRAPH_BUILDER] Grade data found, checking relevance")
            ...
            if isinstance(state.grade, dict):
                logging.info("🔍 [GRAPH_BUILDER] Using dictionary access for grade")
                ...
                logging.info("🔍 [GRAPH_BUILDER] Using object access for grade")
            ...
            logging.info(f"🔍 [GRAPH_BUILDER] Relevance score: {relevance_score}")
            
            if relevance_score is not None and relevance_score > 6:
                # Mark circuit breaker for irrelevant questions
                logging.warning(f"⚠️ [GRAPH_BUILDER] CIRCUIT BREAKER TRIGGERED - Irrelevant question (score: {relevance_score})")
                state.circuit_breaker_triggered = True
                state.relevance_assessment = f"Question not relevant to dataset (relevance score: {relevance_score})"
                return "circuit_breaker"
        
        logging.info(f"🔍 [GRAPH_BUILDER] Question is relevant, checking enrichment need")
        logging.info(f"🔍 [GRAPH_BUILDER] state.should_enrich: {getattr(state, 'should_enrich', 'not set')}")
        
        # For relevant questions, check if enrichment is needed
        if state.should_enrich:
            logging.info("✅ [GRAPH_BUILDER] ROUTING TO ENRICHMENT")
            return "enrich"
        else:
            logging.info("✅ [GRAPH_BUILDER] ROUTING TO INTERPRETATION (no enrichment needed)")
            return "no_enrich"

    def query_execution_successful(self, state: SQLAssistantState):
        logging.info("🔍 [GRAPH_BUILDER] Evaluating query execution success")
        
        # CRITICAL: Check circuit breaker FIRST - both from state and recent messages
        if getattr(state, 'circuit_breaker_triggered', False):
            logging.warning("⚠️ [GRAPH_BUILDER] Circuit breaker already triggered in state")
            ...
            if hasattr(latest_message, 'content'):
                try:
                    ...
                    if isinstance(content, dict) and content.get('circuit_breaker_triggered'):
                        logging.warning("⚠️ [GRAPH_BUILDER] Circuit breaker found in latest message")
                        return "circuit_breaker_end"
                except:
                    pass
        ...
        logging.info(f"🔍 [GRAPH_BUILDER] Retry status: {retry_count}/{max_retries}")
        
        if retry_count >= max_retries:
            logging.warning(f"⚠️ [GRAPH_BUILDER] Retry limit exceeded ({retry_count}/{max_retries})")
            return "circuit_breaker_end"
        
        # Check execution status
        last_execution_failed = getattr(state, 'last_execution_failed', False)
        logging.info(f"🔍 [GRAPH_BUILDER] Last execution failed: {last_execution_failed}")
        
        if last_execution_failed:
            logging.info("🔄 [GRAPH_BUILDER] ROUTING TO FAILURE ANALYSIS")
            return "analyse_failure"
        else:
            logging.info("✅ [GRAPH_BUILDER] ROUTING TO RESULTS PROCESSING")
    ...
    def all_objectives_fulfilled(self, state: SQLAssistantState):
        ...
        """
        logging.info("🔍 [GRAPH_BUILDER] Evaluating if all objectives are fulfilled")
        
        # CRITICAL: Circuit breaker check first
        if getattr(state, 'circuit_breaker_triggered', False):
            logging.warning("⚠️ [GRAPH_BUILDER] Circuit breaker triggered - ending workflow")
        ...
        logging.info(f"🔍 [GRAPH_BUILDER] Iteration counts:")
        ...
        if query_generation_count >= 4 or process_results_count >= 4:
            logging.warning(f"⚠️ [GRAPH_BUILDER] Safety valve triggered - too many iterations")
            return "all_objectives_met"
        ...
        if (hasattr(state, 'objectives_met') and state.objectives_met == "all_objectives_met") or \
           (hasattr(state, 'query_observation_report') and state.query_observation_report and 
            ...
            logging.info("✅ [GRAPH_BUILDER] Explicit objectives met signal found")
            return "all_objectives_met"
        ...
        logging.info(f"🔍 [GRAPH_BUILDER] Has meaningful results: {has_meaningful_results}")
        ...
        if has_meaningful_results:
            ...
            if query_generation_count >= 1:
                logging.info("✅ [GRAPH_BUILDER] Sufficient value achieved - ending workflow")
                return "all_objectives_met"
        ...
        if (hasattr(state, 'objectives_met') and state.objectives_met in [None, "some_objectives_met"]) or \
           (hasattr(state, 'query_observation_report') and state.query_observation_report and 
            ...
            if state.can_retry_objectives:
                ...
                if next_strategy:
                    logging.info(f"🔄 [GRAPH_BUILDER] Attempting objective retry with strategy: {next_strategy}")
                ...
                logging.info("⚠️ [GRAPH_BUILDER] Objective retry limit reached, ending workflow")
                return "all_objectives_met"
        ...
        if (hasattr(state, 'objectives_met') and state.objectives_met == "some_objectives_met") or \
           (hasattr(state, 'query_observation_report') and state.query_observation_report and 
            ...
            if query_generation_count >= 2 and has_meaningful_results:
                logging.info("✅ [GRAPH_BUILDER] Some objectives met with meaningful results - ending workflow")
                return "all_objectives_met"
        ...
        if getattr(state, 'alternative_suggestions_provided', False):
            logging.info("✅ [GRAPH_BUILDER] Alternative suggestions provided - ending workflow")
            return "all_objectives_met"
        ...
        if (state.exploration_findings and 
            ...
            logging.info("✅ [GRAPH_BUILDER] Comprehensive exploration completed - ending workflow")
            return "all_objectives_met"
        
        # Default: continue trying (but limited by safety valves above)
        logging.info("🔄 [GRAPH_BUILDER] Continuing workflow - regenerating query")
        return "regen_query"
    
    def _has_sufficient_value(self, state: SQLAssistantState) -> bool:
        """
        Determine if the current state provides sufficient value to the user.
        This is the key "good enough" detection logic.
        """
        logging.info("🔍 [GRAPH_BUILDER] Evaluating sufficient value")
        ...
        if hasattr(state, 'query_observation_report') and state.query_observation_report:
            ...
            logging.info(f"🔍 [GRAPH_BUILDER] Meaningful insights: {len(meaningful_insights)}")
            
            # If we have 3+ substantial insights, that's valuable
            if len(meaningful_insights) >= 3:
                logging.info("✅ [GRAPH_BUILDER] Sufficient insights found")
                return True
            ...
            if (state.execution_results and 
                ...
                logging.info("✅ [GRAPH_BUILDER] Data results with insights found")
                return True
            ...
            if next_step and len(next_step) > 100:
                logging.info("✅ [GRAPH_BUILDER] Detailed next step found")
                return True
        
        # Check if we have actionable results
        if getattr(state, 'has_actionable_results', False):
            logging.info("✅ [GRAPH_BUILDER] Actionable results found")
            return True
        
        # Check if we have substantial exploration findings
        if (state.exploration_findings and 
            len(state.exploration_findings) > 300):  # Comprehensive exploration
            logging.info("✅ [GRAPH_BUILDER] Substantial exploration findings found")
            return True
        
        logging.info("❌ [GRAPH_BUILDER] Insufficient value - need more work")
        return False

    def build(self, checkpointer: Any):
        logging.info("🏗️ [GRAPH_BUILDER] Building SQL analysis graph")
        ...
        logging.info("🏗️ [GRAPH_BUILDER] Adding nodes and edges to graph")
        ...
        graph_builder.add_conditional_edges(
            ...
            self.query_execution_successful,
            {
                ...
                "circuit_breaker_end": "final_answer_node"  # FIXED: Route through final answer node
            },
        )
        ...
        logging.info("✅ [GRAPH_BUILDER] Graph construction completed")
