    ...
    async def __call__(self, state: SQLAssistantState, config: RunnableConfig):
        ...
        try:
            ...
            if enriched_question and enriched_question != state.input:
                ...
                logging.info("🔍 [INTERPRET_NODE] Using original question (no enrichment available or same as input)")
            ...
            logging.info(f"🔍 [INTERPRET_NODE] Raw LLM response: {response.content}")
            
            response_json = json.loads(response.content)
            interpretation = QueryUnderstanding(**response_json)
            ...
            result_json = interpretation.model_dump_json()
            logging.info(f"🔍 [INTERPRET_NODE] Sending result JSON: {result_json}")
            result_message = AIMessage(content=result_json)
            ...
            # Return fallback interpretation with valid JSON
            ...
            fallback_json = fallback_interpretation.model_dump_json()
            logging.error(f"❌ [INTERPRET_NODE] Returning fallback JSON: {fallback_json}")
